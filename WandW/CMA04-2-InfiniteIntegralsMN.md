...
Then \\( \int_a^\infty \left\{\int_A^\alpha f(x,\alpha')\, d\alpha' \right\} d x\\) converges, that is \\(\int_a^\infty \left\{ \phi(x,\alpha)-\phi(x,A) \right\}\\) converges, and therefore, since \\(\int_a^\infty f(x,\alpha)\,dx\\) converges, so does \\(\int_a^\infty f(x,A)\,dx\\).<a class="marginmark" onClick="toggleHide('mn:12,-4');">&#91;12&#93;</a> 
