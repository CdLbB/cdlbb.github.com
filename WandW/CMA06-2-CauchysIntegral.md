...
\\( \frac{1}{2\pi i} \int_{C} f(z) \frac{\phi'(z)}{\phi(z)} \,dz \\)
...
independent\footnote{\\(\left|\,U\,\right|\\) is a continuous function of \\(z\\) on
  \\(C\\), and so *attains* its upper bound
...
But \\(\int_{C} \frac{d z}{z} = 2 \pi i\\); and, since \\(\left|\,U\,\right| < 1\\),
