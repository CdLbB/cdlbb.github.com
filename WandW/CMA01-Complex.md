...
It is not part of the scheme of this work to discuss the construction of the class of integers or the logical foundations of the theory of rational numbers.[^DefiningReals]
...
>If every rational number belongs either to the class \\((A_1+B_1)\\) or to the class \\((A_2+B_2)\\), then the classes \\((A_1+B_1), (A_2+B_2)\\) define an irrational number.[^assumesIrrational] If one rational number \\(x\\) exists belonging to neither class, then the \\(L\\)-class formed by \\(x\\) and \\((A_1+B_1)\\) and the \\(R\\)-class \\((A_2+B_2)\\) define the rational-real number x. In either case, the number defined is called the sum \\(\alpha + \beta\\).
>
[^assumesIrrational]:*Editor's Note:* This claim does not always hold if \\(\alpha\\) and \\(\beta\\) are both rational, but then the sum is clearly rational. If we assume at least one of \\(\alpha\\) and \\(\beta\\) are irrational, this and the succeeding claim are true.
...
:V. von Dantscher, [*Vorlesungen ueber die Weierstrass’sche Theorie der irrationalen Zahlen.*][Vorlesungen] (Leipzig, 1908).
...
:O. Stolz, [*Vorlesungen über allgemeine Arithmetik*, **II.**][Vorlesungen] (Leipzig, 1886).
